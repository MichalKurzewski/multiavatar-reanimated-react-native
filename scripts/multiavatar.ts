...
export default function multiavatar(
  ...
  const svgParts: Record<string, SPEntry> = {};
  const svgStart = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 231 231">';
  const svgEnd = "</svg>";
  ...
  const strokeStyle = "stroke-linecap:round;stroke-linejoin:round;stroke-width:";
  ...
  hash = simpleHash(string);

  // parts
  const parts: Record<string, number | string> = {};

  // Get parts (range 0-47)
  // Convert two-digit hash to number, then map to 0-47 range
  parts.env = parseInt(hash[0] + hash[1], 10);
  parts.env = Math.round((47 / 99) * parts.env);

  parts.clo = parseInt(hash[2] + hash[3], 10);
  parts.clo = Math.round((47 / 99) * parts.clo);

  parts.head = parseInt(hash[4] + hash[5], 10);
  parts.head = Math.round((47 / 99) * parts.head);

  parts.mouth = parseInt(hash[6] + hash[7], 10);
  parts.mouth = Math.round((47 / 99) * parts.mouth);

  parts.eyes = parseInt(hash[8] + hash[9], 10);
  parts.eyes = Math.round((47 / 99) * parts.eyes);

  parts.top = parseInt(hash[10] + hash[11], 10);
  parts.top = Math.round((47 / 99) * parts.top);

  // Get parts (range 0-15) + define themes
  let partNumber: number = 0;
  let partNumberString: string = "";
  let part = "";
  let partValue: number | string = 0;
  for (part in parts) {
    partValue = parts[part];
    ...
    if (partNumber > 31) {
      partNumber = partNumber - 32;
      partNumberString = partNumber.toString();
      if (partNumberString.length === 1) partNumberString = `0${partNumberString}`;
      parts[part] = `${partNumberString}C`;
    } else if (partNumber > 15) {
      partNumber = partNumber - 16;
      partNumberString = partNumber.toString();
      if (partNumberString.length === 1) partNumberString = `0${partNumberString}`;
      parts[part] = `${partNumberString}B`;
    } else {
      partNumberString = partNumber.toString();
      if (partNumberString.length === 1) parts[part] = `0${partNumberString}A`;
      else parts[part] = `${partNumberString}A`;
    }
  }
  ...
  let partVersion = "";
  let theme = "";
  let partValueString: number | string = "";

  for (part in parts) {
    partValueString = parts[part];
    if (typeof partValueString === "string") {
      partVersion = partValueString.substring(0, 2);
      theme = partValueString.substring(2, 3);
    } else {
      continue;
    }

    if (typeof version !== "undefined") {
      partVersion = version.part;
      theme = version.theme;
    }
    final[part] = getFinal(part, partVersion, theme);
  }

  function getFinal(part: string, partVersion: string, theme: string): string {
    const colors =
      themes[partVersion as keyof typeof themes][theme as "A" | "B" | "C"][
        part as keyof (typeof themes)["00"]["A"]
      ];
    const svgString = (svgParts[partVersion]?.[part as keyof SPEntry] as string) ?? "";
    ...
    if (result !== null) {
      for (let i = 0; i < result.length; i++) {
        finalSvgString = finalSvgString.replace(result[i], `${colors[i]};`);
      }
    }
